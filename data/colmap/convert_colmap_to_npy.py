...
# Converts  COLMAP text ouptut into poses.

def quaternion_to_rotation_matrix(q):
    # q = [qw, qx, qy, qz], unpack quaternion
    qw, qx, qy, qz = q

    # Normalize the quaternion
    norm = np.sqrt(qw*qw + qx*qx + qy*qy + qz*qz)
    
    # Normalize quaternion - make sure it's a unit quat. for accurate rotation.
    qw, qx, qy, qz = qw / norm, qx / norm, qy / norm, qz / norm

    # Standard formula to convert quaternion into 3x3 rotation matrix.
    R = np.array([
...
if __name__ == '__main__':
    ...
    output_dir = 'data/processed/plant/poses'
